...
class IntentHandler: INExtension, INSendMessageIntentHandling, INSearchForMessagesIntentHandling, INSetMessageAttributeIntentHandling {
    ...
    func resolveRideOptionName(forRequestRide intent: INRequestRideIntent,
        ...
        let permier = BikeTaxiClass.premier.description
        let permierSpeakableString = INSpeakableString(identifier: "", spokenPhrase: permier, pronunciationHint: permier)
        ...
        let result = INSpeakableStringResolutionResult.disambiguation(with: [permierSpeakableString,basicSPeakableString])
        
        completion(result)
    }
    
    func confirm(requestRide intent: INRequestRideIntent, completion: (INRequestPaymentIntentResponse) -> Void) {
        
        let responseCode = INRequestPaymentIntentResponseCode.ready
        let response = INRequestPaymentIntentResponse(code: responseCode, userActivity: nil)
...
extension IntentHandler: INRequestRideIntentHandling {
    func handle(requestRide intent: INRequestRideIntent, completion: @escaping (INRequestRideIntentResponse) -> Void) {
        ...
        personComponents.familyName = "Applessed"
        ...
        vehicle.model = intent.rideOptionName?.spokenPhrase
        vehicle.location = CLLocation(latitude: 54.978252, longitude: -1.6177800000)
